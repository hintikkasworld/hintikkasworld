...
export class NanoHanabi extends ExampleDescription {
    ...
    getInitialEpistemicModel() {
        ...
        for (let permutation of permutations) {
            M.addWorld(
                'w' + permutation[0] + permutation[1] + permutation[2] + permutation[3],
                new NanoHanabiWorld(
                    new Valuation(['al' + permutation[0], 'ar' + permutation[1], 'bl' + permutation[2], 'br' + permutation[3], 'c0'])
                )
            );
        }
        ...
        let selected = Math.floor(Math.random() * 24);
