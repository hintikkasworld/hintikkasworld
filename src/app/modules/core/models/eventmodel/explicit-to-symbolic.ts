...
export class ExplicitToSymbolic {
     ...
     */
    static translate(E: ExplicitEventModel, variables: string[], agents: string[]): SymbolicEventModel {

        console.log("==================================")
        console.log("ExplicitToSymbolic.translate");

        let sE = new SymbolicEventModel(agents, variables);

        let event_bdd = new Map();

        for (const [event, value] of <[string, { pre: Formula, post: Postcondition }][]>Object.entries(E.getNodes())) {
            console.log(event, value);
            event_bdd.set(event, ExplicitToSymbolic._event_to_bdd(value.pre, value.post));
            //console.log(event_bdd[event]);
            sE.addUniqueEvent(event, event_bdd.get(event));
            console.log(event, BDD.bddService.pickAllSolutions(event_bdd.get(event)));
        }

        console.log("Unique Event OK", E.getAgents());
        ...
        for (let agent of agents) {
            ...
            for (let node in E.getNodes()) {
                ...
                for (let succ of E.getSuccessorsID(node, agent)) {
                    // console.log("Succ = ", succ)
                    // console.log(BDD.bddService.pickAllSolutions(event_bdd.get(succ)))
                    // console.log("  Succ", succ, event_bdd.get(succ), BDD.bddService.pickAllSolutions(event_bdd.get(succ)));

                    let toprime = SymbolicEpistemicModel.getMapNotPrimeToPrime(BDD.bddService.support(event_bdd.get(succ)));
                ...
                sE.addPlayerEvent(node, agent, new_or_action)

            }
        }

        sE.setPointedAction(E.getPointedAction());
    ...
    static _frame(vars: string[], prime: boolean): BDDNode {
        ...
        // console.log("end frame", vars, prime, BDD.bddService.pickAllSolutions(pointeur));
        return pointeur;
    }
