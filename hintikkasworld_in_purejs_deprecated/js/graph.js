...
class Graph
{
  ...
  getSuccessors(node, agent) {
      if(!isAgent(agent)) throwError("The second argument " + agent + " of getSuccessors should be an agent. " + ERROR_AGENT_EXPLANATION);
      if(this.nodes[node] == undefined) throwError( "getSuccessors : There is no source node of ID " + node);
...
*/
  ...
  */
    ...
    if(this.nodes[idnode] != undefined)
      throwError( "The structure already contains a node of ID " + idnode);
  ...
  */
	addEdge(agent, idsource, iddestination) {
        if(!isAgent(agent)) throwError( "The first argument " + agent + " of addEdge should be an agent. " + ERROR_AGENT_EXPLANATION);
        if(this.nodes[idsource] == undefined) throwError("There is no source node of ID " + idsource);
        if(this.nodes[iddestination] == undefined) throwError("There is no destination node of ID " + iddestination);
	...
	**/
	setPointedNode(w) {
		if(this.nodes[w] == undefined)
			throwError("the graph does not contain any node of ID " + w);
		this.pointed = w;
	}
